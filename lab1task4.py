...
def get_chain_from_word(word,alphabet):
    return [alphabet] + [rotate(alphabet,alphabet.index(c) % len(alphabet)) for c in word.upper()]
...
chain = get_chain_from_word(word,alphabet)
